...
async function composeFullVideo(
  ...
  if (completedScenes.length === 0) {
    // Previously this was a silent `return`, which left the order stuck
    // forever with no errorMessage and no MP4 in S3. Throw instead so the
    // outer `scheduleVideoGeneration` catch records `errorMessage` on the
    // order and the UI can surface it / offer a retry button.
    const totalScenes = allScenes.length;
    const failedScenes = allScenes.filter((s) => s.status === "failed").length;
    const detail =
      totalScenes === 0
        ? "no scene records exist for this order"
        : `${failedScenes}/${totalScenes} scenes failed and none completed with an illustration`;
    console.error(
      `[VideoJob] Cannot compose video for order ${orderId}: ${detail}`
    );
    throw new Error(
      `No usable scenes to compose into a video (${detail}). ` +
        `Try regenerating the storybook from the Storybook page.`
    );
  }
  ...
  // Upload to S3.
  //
  // ContentDisposition: "attachment; filename=..." is set on the S3 object
  // itself so that the direct public URL also triggers a file download in
  // the browser instead of rendering the MP4 inline. Without this header,
  // a user clicking the public videoUrl would just see the video play in a
  // new tab with no obvious way to save it. The pre-signed download URL
  // generated by the /getVideoDownloadUrl mutation already overrides this
  // header per-request, so this is a robust default for every other
  // place the URL might be linked from (email, share page, etc.).
  const ns = getStorageNamespace(userId, orderId);
  const videoKey = `${ns}/videos/${nanoid()}-storybook.mp4`;
  // Build a friendly filename based on the child's name. Mirrors the
  // logic in routers/orders.ts so the downloaded file looks the same
  // whether the user uses the button or right-clicks the public URL.
  const safeChildName = (childName || "storybook")
    .replace(/[^a-z0-9]/gi, "-")
    .toLowerCase();
  const downloadFilename = `${safeChildName}-animated-story.mp4`;
