...
export default function StorybookPage() {
  ...
  const getVideoDownloadUrl = trpc.orders.getVideoDownloadUrl.useMutation({
    onSuccess: (data) => {
      // Trigger the download via a hidden anchor element instead of
      // window.location.href. Navigating the top-level page is unreliable —
      // some browsers (especially mobile Safari/Chrome) treat the response
      // as a navigation and end up showing a blank page when they see
      // Content-Disposition: attachment, or block it entirely. A
      // programmatic <a download> click stays on the current page and
      // works consistently across desktop and mobile browsers.
      try {
        const link = document.createElement("a");
        link.href = data.downloadUrl;
        link.download = data.filename || "storybook.mp4";
        // target="_blank" gives mobile browsers a tab to perform the
        // download in without disturbing the current view.
        link.target = "_blank";
        link.rel = "noopener noreferrer";
        document.body.appendChild(link);
        link.click();
        // Defer removal so the click has time to register on slower devices.
        setTimeout(() => {
          if (link.parentNode) link.parentNode.removeChild(link);
        }, 100);
        toast.success("Download started! Check your downloads folder.");
      } catch (err) {
        // Last-resort fallback: open in a new tab so the user at least
        // has a way to save the file via the browser's context menu.
  ...
  // Direct-link fallback: a plain Express endpoint that 302-redirects to
  // a pre-signed S3 URL with attachment headers. Useful when JavaScript
  // fails or for users who prefer right-click → "Save link as".
  const directDownloadHref = orderId
    ? `/api/download/video/${orderId}${
              ...
              <Card className="bg-gradient-to-r from-purple-900/60 to-pink-900/60 border-purple-500/40">
                <CardContent className="pt-6">
                  ...
                  <div className="rounded-xl overflow-hidden bg-black">
                    {/* Note: we deliberately do NOT pass controlsList="nodownload"
                        so the browser's built-in three-dot menu still shows a
                        Download option as a backup on Chrome/Edge. */}
                    <video
                      src={order.videoUrl}
                      controls
                      autoPlay={false}
                  ...
                  <div className="mt-4 flex flex-col sm:flex-row gap-3 justify-center items-center">
                    <Button
                      ...
                      className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-5 py-2.5 font-semibold"
                    >
                    ...
                    {/* Plain anchor fallback — works even if JS fails or the
                        button click is blocked. The /api/download/video
                        endpoint redirects to a pre-signed S3 URL with
                        Content-Disposition: attachment, so the browser
                        downloads instead of navigating. */}
                    <a
                      href={directDownloadHref}
                      download
                      className="text-xs text-purple-200/80 hover:text-white underline underline-offset-2"
                    >
                      Having trouble? Use direct download link
                    </a>
                  </div>
                  <p className="text-xs text-purple-300/60 mt-3 text-center">
                    Your personalized animated storybook is ready! Tap "Download
                    Video" to save the MP4 to your device, or right-click the
                    video to use your browser's save option.
                  </p>
                </CardContent>
              </Card>
            )}
