...
export default function CheckoutPage() {
  ...
  const [paymentPendingLong, setPaymentPendingLong] = useState(false);
  ...
  // confirmPayment mutation — queries Stripe directly, no webhook dependency
  const confirmPaymentMutation = trpc.orders.confirmPayment.useMutation({
    onSuccess: (data) => {
      if (data.confirmed) {
        setPaymentPending(false);
        setPaymentPendingLong(false);
        toast.success("Payment confirmed! Your storybook is being generated.");
        refetch();
      }
      // If not confirmed yet, polling will continue
    },
    onError: () => {
      // Stripe query failed — fall back to webhook polling, don't stop spinner
    },
  });
  ...
  useEffect(() => {
    if (paymentResult === "success") {
      ...
      // Extract session_id from URL and call confirmPayment immediately
      ...
      // Poll every 3s as fallback (webhook may arrive later)
      const pollInterval = setInterval(() => {
        refetch();
      }, 3000);
      // After 15s show a "taking longer than expected" message
      const longTimeout = setTimeout(() => {
        setPaymentPendingLong(true);
      }, 15000);
      // After 90s give up polling
                    ...
                    {paymentPendingLong ? (
                      <>
                        <p className="text-yellow-700 mb-4">
                          This is taking a little longer than usual. Your payment was received — we're waiting for Stripe to confirm.
                        </p>
                        <Button
                          variant="outline"
                          size="sm"
                        ...
                        <p className="text-xs text-yellow-600 mt-3">
                          If this persists, please check your email for a payment receipt and contact support.
                        </p>
                      </>
                    ) : (
                      <>
