...
export default function StorybookPage() {
  ...
  const hasVideoError = !hasServerVideo && !!(order?.errorMessage && order.errorMessage.toLowerCase().includes("video generation failed"));
  // Video is generating if: server reports it in progress, OR user just clicked the button
  // BUT NOT if there's a video error (show error UI instead) or video already exists
  const videoGenerating = !hasServerVideo && !hasVideoError && (order?.videoGenerating || videoGenTriggered || triggerVideo.isPending);
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {(orderLoading || scenesLoading) && !order && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-slate-400">Loading your storybook...</p>
          </div>
        )}
        ...
        {order && !orderError && (
          <div className="space-y-6">
            ...
            {order.paymentStatus === "paid" && order.storyApproved && !hasScenes && (
              <Card className="bg-slate-800/50 border-purple-500/30">
                <CardContent className="pt-6 text-center">
                  {videoGenerating ? (
                    <>
            ...
            {/* Video Generation Section - for paid orders with all scenes complete */}
            {order?.paymentStatus === "paid" && allScenesCompleted && !hasServerVideo && (
