 ...
 * Strategy (optimized for Cloud Run — npm package first, no slow subprocess spawning):
 *   1. ffmpeg-static / ffprobe-static npm packages (bundled, no download needed, fastest)
 *   2. System-installed ffmpeg/ffprobe (dev sandbox, Linux servers with ffmpeg installed)
 *   3. Common paths (/usr/bin/ffmpeg, etc.)
 *   4. Cached auto-downloaded binary from previous run
 */
function resolveFFmpegPathsSync(): { ffmpeg: string; ffprobe: string } | null {
  // Strategy 1: Try ffmpeg-static npm packages FIRST (bundled binary, works on Cloud Run)
  // These are top-level ESM imports — no subprocess spawning, instant resolution
  try {
    const staticFfmpeg = ffmpegStaticPath as string | null;
    const staticFfprobe = (ffprobeStaticPkg as { path: string } | null)?.path;
  ...
  // Strategy 2: Try system ffmpeg (dev sandbox / Linux servers)
  try {
    ...
    console.log("[VideoCompose] System ffmpeg not available, trying common paths...");
  }
...
export async function composeStorybookVideo(
  ...
  try {
    ...
    for (let i = 0; i < sceneFiles.length; i++) {
      ...
      if (audioPath) {
        // Scene with narration: scale image to 1920x1080 with black letterbox bars,
        // display for the narration duration. Simple and reliable on Cloud Run.
        await runFFmpeg(
          ...
          `-t ${duration.toFixed(2)} -shortest -pix_fmt yuv420p "${clipPath}"`,
          120000
        );
      } else {
        ...
        await runFFmpeg(
          ...
          `-vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:black,setsar=1" ` +
          `-c:v libx264 -preset fast -crf 23 -c:a aac -b:a 128k ` +
          `-t ${duration.toFixed(2)} -pix_fmt yuv420p "${clipPath}"`,
          120000
        );
      }
