...
export default function SharedStorybookPage() {
  ...
  const getDownloadUrl = trpc.orders.getSharedStorybookDownloadUrl.useMutation({
    onSuccess: (data) => {
      // Use a programmatic anchor click instead of window.location.href so
      // the page doesn't navigate away. Some browsers (especially mobile
      // Safari/Chrome) handle Content-Disposition: attachment poorly when
      // delivered via a top-level navigation and end up showing a blank page.
      try {
